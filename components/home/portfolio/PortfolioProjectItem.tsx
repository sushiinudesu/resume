...
export default function PortfolioProjectItem({
  ...
  return (
    <li key={project.id}>
      ...
      <div className="ml-4 md:ml-8 max-w-3xl rounded-lg border border-zinc-200/70 dark:border-zinc-800/80 bg-white/70 dark:bg-zinc-900/70 p-4">
        <div className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-start gap-x-4 gap-y-2">
          ...
          {projectRole && (
            <p className="col-start-2 text-sm text-zinc-600 dark:text-zinc-400">{`${t('role')}: ${projectRole}`}</p>
          )}
          <PortfolioProjectTech tech={project.tech ?? []} />
          {projectDesc && (
            <p className="col-start-2 -col-end-1 mt-2 max-w-prose whitespace-pre-line border-l-4 border-zinc-400/70 dark:border-zinc-500 px-3 py-2 text-[15px] leading-7 font-medium tracking-[0.01em] text-zinc-800 dark:text-zinc-200">{projectDesc}</p>
          )}
        </div>
      </div>
    </li>
